		...
		if($provinces->count() > 0){
			foreach ($provinces as $province) {
				if($province->provinceid == $select){
					$html .= '<option value="'.$province->provinceid.'" selected="">'.str_replace(array('Tỉnh','Thành phố'), '', $province->name).'</option>';
				}else{
					$html .= '<option value="'.$province->provinceid.'">'.str_replace(array('Tỉnh','Thành phố'), '', $province->name).'</option>';
				}
			}
		}
		return $html;
