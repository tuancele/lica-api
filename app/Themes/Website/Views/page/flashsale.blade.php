...
<section class="products pb-5">
    <div class="container-lg">
        ...
        {{$products->links()}}
        @else
        <div class="mt-5">Không có chương trình khuyến mại nào</div>
        @endif
        </div>
...
<script>
    $('.sort').on('click','a',function(){
        ...
        $.ajax({
            type: 'post',
            url: '/load-sort',
            data: {sort:sort,url:url,cat_id:'',page:'flashsale'},
            headers:
            {
    ...
    $(".list-filter li input").click(function () {
        ...
        $.ajax({
            type: 'post',
            url: '/load-filter',
            data: {origin:origin,brand:brand,color:color,size:size,url:url,cat_id:'',price:price,page:'flashsale',orderby:''},
            headers:
            {
              'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
