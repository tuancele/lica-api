        ...
        <div class="des-quick">
            ...
            <div class="group-cart product-action align-center mt-3 space-between">
                <div class="quantity align-center quantity-selector">
                    @php
                        // Warehouse V2: Use stock_display from first variant (if available) or check detail stock
                        $quickStockDisplay = isset($first) && isset($first->stock_display) 
                            ? $first->stock_display 
                            : (isset($first) && isset($first->warehouse_stock) 
                                ? $first->warehouse_stock 
                                : ($detail->stock ?? 0));
                        $isQuickOutOfStock = $quickStockDisplay <= 0;
                    @endphp
                    <button class="btn_minus entry" type="button" @if($isQuickOutOfStock) disabled @endif>
                    ...
                    <input @if($isQuickOutOfStock) disabled @endif type="text" class="form-quatity quantity-input" value="1" min="1">
                    <button @if($isQuickOutOfStock) disabled @endif class="btn_plus entry" type="button">
                ...
                <div class="item-action">
                    <button @if($isQuickOutOfStock) disabled @endif type="button" class="addCartDetail">
