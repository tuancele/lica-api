...
@section('title', 'Tìm kiếm: '.request()->s)
@section('description', 'Tìm kiếm: '.request()->s)
@section('content')
<section class="mt-3">
    <div class="container-lg">
        <div class="breadcrumb">
            <ol>
                ...
                <li><a href="/tim-kiem?s={{request()->s}}">Tìm kiếm</a></li>
            </ol>
        </div>
        <h1 class="text-uppercase title-cate">Kết quả tìm kiếm</h1>
    </div>
</section>
...
<script>
    $('.sort').on('click','a',function(){
        ...
        var url = '/tim-kiem?s={{request()->s}}';
        var text = $(this).text();
        $.ajax({
            type: 'post',
            url: '/load-sort',
            data: {sort:sort,url:url,cat_id:'{{request()->s}}',page:'search'},
            headers:
            {
    ...
    $(".list-filter li input").click(function () {
        ...
        $.ajax({
            type: 'post',
            url: '/load-filter',
            data: {origin:origin,brand:brand,color:color,size:size,url:url,cat_id:'{{request()->s}}',price:price,page:'search',orderby:''},
            headers:
            {
              'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
