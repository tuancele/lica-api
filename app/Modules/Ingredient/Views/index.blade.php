...
<div class="box">
    <div class="box-header with-border">
        <div class="row">
            <div class="col-md-9">
                <div class="row">
                    <form method="get" action="{{route('ingredient')}}"> 
            ...
            <div class="col-md-3">
                <a class="button add btn btn-info pull-right" href="{{route('ingredient.create')}}"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
            </div>
        </div>
    </div><!-- /.box-header -->
    <div class="box-body">
        <form id="tblForm" method="post" delete-url="{{route('ingredient.delete')}}" action-url="{{route('ingredient.action')}}">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$value->name}}
                                @if($value->link != "")<p style="margin-bottom: 0px;">
                                    <a href="/{{$value->link}}" target="_blank">{{asset($value->link)}}</a></p>@endif
                            </td>
                            <td>{{date('d/m/Y',strtotime($value->created_at))}}</td>
                            <td>@if(isset($value->user)){{$value->user->name}}@endif</td>
                            <td>
                                <select class="select_status form-control" data-id="{{$value->id}}" data-url="{{route('ingredient.status')}}">
                            ...
                            <td>
                                <a class="btn btn-primary btn-xs" href="{{route('ingredient.edit',['id' =>$value->id])}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
