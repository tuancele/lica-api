...
class RedirectionController extends Controller
{
    ...
    public function show(int $id): JsonResponse
    {
        try {
            ...
            if (! $redirection) {
                return response()->json(['success' => false, 'message' => 'Redirection not found'], 404);
            }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        try {
            ...
            $updateData = [];
            if ($request->has('link_from')) {
                $updateData['link_from'] = $request->link_from;
            }
            if ($request->has('link_to')) {
                $updateData['link_to'] = $request->link_to;
            }
            if ($request->has('type')) {
