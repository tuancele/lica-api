...
<div class="box">
    <div class="box-header with-border">
        <div class="row">
            ...
            <div class="col-md-3">
                <a class="button add btn btn-info pull-right" href="{{route('brand.create')}}"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
            </div>
        </div>
    </div><!-- /.box-header -->
    <div class="box-body">
        <form id="tblForm" method="post" delete-url="{{route('brand.delete')}}" action-url="{{route('brand.action')}}">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><img src="{{getImage($value->image)}}" alt="" width="100px"></td>
                            <td><a href="/thuong-hieu/{{$value->slug}}" target="">{{$value->name}}</a></td>
                            <td>{{date('d/m/Y',strtotime($value->created_at))}}</td>
                            <td>@if(isset($value->user)){{$value->user->name}}@endif</td>
                            <td>
                                <select class="select_status form-control" data-id="{{$value->id}}" data-url="{{route('brand.status')}}">
                            ...
                            <td>
                                <a class="btn btn-primary btn-xs" href="{{route('brand.edit',['id' =>$value->id])}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
