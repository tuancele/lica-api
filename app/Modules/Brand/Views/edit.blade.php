    ...
    <form role="form" id="tblForm" method="post" ajax="{{route('brand.update')}}">
        ...
        <div class="row">
                    ...
                    <div class="panel-body">
                        <div class="list_image row">
                            ...
                            @foreach($gallerys as $ga => $gallery)
                                <div class="col-md-3 item{{$ga+1}}"><img src="{{$gallery}}"><input type="hidden" value="{{$gallery}}" name="imageOther[]"><a data-id="{{$ga+1}}" href="javascript:;" title="Xóa ảnh" class="delete_image"><i class="fa fa-times"   aria-hidden="true"></i></a></div>
                            @endforeach
                            @endif
                        </div>
                    </div>
            ...
            <div class="col-lg-3">
                ...
                <div class="panel panel-default">
                    @include('Layout::image',['image' => $detail->logo,'number' => 1,'title' => 'Logo','name' => 'logo'])
                </div>
                <div class="panel panel-default">
                    @include('Layout::image',['image' => $detail->image,'number' => 2,'title' => 'Ảnh đại diện','name' => 'image'])
                </div>
                <div class="panel panel-default">
                    @include('Layout::image',['image' => $detail->banner,'number' => 3,'title' => 'Banner','name' => 'banner'])
                </div>
            </div>
            <!-- /.col-lg-12 -->
        </div>
