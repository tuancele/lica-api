...
<section class="content">
    <form role="form" id="tblForm" method="post" ajax="/admin/export-goods/create">
        @csrf
        <div class="row">
            <!-- Thông tin đơn hàng -->
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Nội dung xuất <span class="text-red">*</span></label>
            ...
            <div class="col-md-12">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-cubes"></i> Sản phẩm xuất</h3>
            ...
            <div class="col-md-12">
                <div class="box-footer">
                    <button type="submit" class="btn btn-success btn-lg">
                        <i class="fa fa-save"></i> Lưu đơn xuất hàng
                    </button>
                    <a href="/admin/export-goods" class="btn btn-default btn-lg">
...
$(document).ready(function() {
    ...
    $('.select_product').select2({
        ...
        ajax: {
            url: '/admin/export-goods/searchProducts',
            dataType: 'json',
            delay: 300,
    ...
    $('#listProduct').on('change', '.select_product', function(){
        ...
        if(productId && productId != '0'){
            $.ajax({
                type: 'get',
                url: '/admin/export-goods/getVariants/'+productId,
                success: function (res) {
    ...
    $('#listProduct').on('change', '.select_variant', function(){
        ...
        if(variantId && variantId != ''){
            ...
            $.ajax({
                type: 'get',
                url: '/admin/export-goods/getVariantStock/'+variantId,
                success: function(res) {
                    if(res.stock !== undefined) {
                        var stock = res.stock;
                        var stockText = '<i class="fa fa-cubes"></i> Tồn kho: <strong>'+stock+'</strong>';
                        if(stock <= 0) {
                            stockText += ' <span class="text-red">(Hết hàng!)</span>';
                            $variantInfo.addClass('stock-warning');
                        } else {
                            $variantInfo.removeClass('stock-warning');
                        }
                        $variantInfo.html(stockText).show();
                    }
                }
            });
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res) {
                        $row.find('.input-price').val(formatNumber(res)).trigger('input');
                    }
                }
            });
    ...
    $('#listProduct').on('input', '.input-price, .input-qty', function(){
        ...
        if($input.hasClass('input-qty')) {
            var variantId = $row.find('.select_variant').val();
            if(variantId) {
                checkStockAvailability($row, variantId, qty);
            }
        }
    });
    
    // Check stock availability
    function checkStockAvailability($row, variantId, qty) {
        $.ajax({
            type: 'post',
            url: '/admin/export-goods/checkTotal',
            data: {qty: qty, productid: variantId},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function(res) {
                ...
                if(res === false) {
                    $qtyInput.css('border-color', '#d9534f');
                    $qtyInput.css('background-color', '#f2dede');
                } else {
    ...
    $('#tblForm').on('submit', function(e) {
        ...
        $('#listProduct tbody tr.item_product').each(function() {
            ...
            var qty = parseNumber($(this).find('.input-qty').val());
            
            if(productId && productId != '0' && variantId && variantId != '') {
                hasProducts = true;
                
                // Check stock synchronously before submit
                ...
                if(!stockCheck) {
                    var productName = $(this).find('.select_product option:selected').text();
                    stockErrors.push(productName + ': Số lượng vượt quá tồn kho');
                }
            }
        });
