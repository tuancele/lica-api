...
{
    private $model;
    private $controller = 'selling';
    private $view = 'Selling';
    public function __construct(Selling $model){
        $this->model = $model;
    }
    public function index(Request $request)
    {
        active('media','selling');
        $data['list'] = $this->model::where('type','selling')->where(function ($query) use ($request) {
    ...
    public function create(){
        active('media','selling');
        $data['products'] = Product::select('id','name')->where([['status','1'],['type','product']])->get();
