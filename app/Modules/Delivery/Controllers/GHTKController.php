...
class GHTKController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($result->success) {
            Order::where('id', $detail->id)->update([
                'ship' => '1',
            ...
            if (getConfig('delivery_mail_status')) {
                ...
                $a_search = ['{site_title}', '{ship_id}', '{order_id}', '{estimated_deliver}'];
                $replace = [getConfig('site_name'), $result->order->label, $detail->code, $result->order->estimated_deliver_time];
    ...
    public function testSendmail()
    {
        $subject = getConfig('delivery_mail_title');
        $a_search = ['{site_title}', '{ship_id}', '{order_id}', '{estimated_deliver}'];
        $replace = [getConfig('site_name'), 'ABc123', '1232132', '20/05/2023'];
        $subject = str_replace($a_search, $replace, $subject);
        $body = [
            'title' => $subject,
            'content' => str_replace($a_search, $replace, getConfig('delivery_mail_content')),
