...
<div class="box">
    <div class="box-header with-border">
        <div class="row">
            ...
            <div class="col-md-2">
                 @can('role-create')
                <a class="button add btn btn-info pull-right" href="/admin/role/create" style="margin-right:5px;"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
                @endcan
            </div>
        </div>
    </div><!-- /.box-header -->
    <div class="box-body">
        <form id="tblForm" method="post" delete-url="/admin/role/delete" action-url="/admin/role/action">
            <div class="PageContent">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td> @if($value->id != 1)<input type="checkbox" name="checklist[]" class="checkbox wgr-checkbox" value="{{$value->id}}">@endif</td>
                            <td>
                               {{$value->name}}</a>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($value->id != 1)
                                <select class="select_status form-control" data-id="{{$value->id}}" data-url="/admin/role/status">
                            ...
                            <td>
                                @if($value->id != 1)
                                <a class="btn_delete btn btn-danger btn-xs pull-right" data-page="{{ app('request')->input('page') }}" data-id="{{$value->id}}"><i class="fa fa-trash-o" aria-hidden="true"></i> Xóa</a>
                                <a class="btn btn-primary btn-xs pull-right" href="/admin/role/edit/{{$value->id}}" style="margin-right:3px"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Sửa</a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
