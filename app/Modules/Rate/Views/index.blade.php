...
<div class="box">
    <div class="box-header with-border">
        <div class="row">
            <div class="col-md-9">
                <div class="row">
                    <form method="get" action="/admin/rate"> 
                    ...
                    <div class="col-md-2 pr-0">
                        <?php $product_id = request()->get('product_id'); ?>
                        <select class="form-control" name="product_id">
                            <option value=""  @if($product_id == "") selected="" @endif>---Sản phẩm---</option>
                            @if($products->count() > 0)
                            @foreach($products as $product)
                            <option value="{{$product->id}}" @if($product->id == $product_id && $product_id != "") selected="" @endif>{{$product->name}}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
            ...
            <div class="col-md-3">
                <a class="button add btn btn-info pull-right" href="/admin/rate/create"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
            </div>
        </div>
    </div><!-- /.box-header -->
    <div class="box-body">
        <form id="tblForm" method="post" delete-url="/admin/rate/delete" action-url="/admin/rate/action">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$value->name}}<p style="color:orange;margin-bottom:0px">{!!getStarAdmin($value->rate)!!}</p>
                                @if($value->product)<p><a href="/{{$value->product->slug}}" target="_blank">{{$value->product->name}}</a></p>@endif
                            </td>
                            <td>{{$value->phone}}</td>
                            <td>{{$value->email}}</td>
                            <td>{{$value->content}}</td>
                            <td>
                                <select class="select_status form-control" data-id="{{$value->id}}" data-url="/admin/rate/status">
                            ...
                            <td>
                                <a class="btn btn-primary btn-xs" href="/admin/rate/edit/{{$value->id}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
